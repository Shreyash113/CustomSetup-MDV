#!/bin/bash
set -e  # Exit if any command fails (we use || true where non-fatal)
...
# Optional: pin these so you can change without editing the script
TRITON_VERSION="${TRITON_VERSION:-3.2.0}"
SAGEATTN_VERSION="${SAGEATTN_VERSION:-1.0.6}"

# HuggingFace workflows (repo public; token optional)
HF_WORKFLOWS_REPO="${HF_WORKFLOWS_REPO:-Shreyash113/workflows}"
HF_WORKFLOWS_SUBDIR="${HF_WORKFLOWS_SUBDIR:-}"
...
export_env_vars() {
    ...
    # Avoid duplicate lines across restarts
...
# ---------------------------------------------------------------------------- #
#            Auto-download models from MODEL_LIST_URL (TXT manifest)            #
# ---------------------------------------------------------------------------- #
auto_download_models_txt() {
    ...
    if command -v curl >/dev/null 2>&1; then
        ...
        # ✅ FIXED: was incorrectly "$TMP_LIST"
...
# ---------------------------------------------------------------------------- #
#                Sync HuggingFace workflows into ComfyUI folder                 #
# ---------------------------------------------------------------------------- #
sync_hf_workflows() {
...
# Custom nodes: every boot (only missing)
install_custom_nodes_every_boot

# Auto model download (after venv active)
auto_download_models_txt

# Workflow sync (non-fatal)
